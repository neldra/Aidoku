...
/// holds only `SystemSpeechBackend`; `KokoroSpeechBackend` is constructed only
/// inside `if #available(iOS 16, *)`, so iOS 15 users never see Kokoro.
@MainActor
final class SpeechBackendRegistry {
    ...
    /// The Kokoro model manager, stored type-erased so this non-gated class can
    /// hold an `@available(iOS 16, *)` type. Reach it via `kokoroModelManager`.
    ...
    convenience init() {
        ...
        if #available(iOS 16, *) {
            ...
            let kokoro = KokoroSpeechBackend(modelManager: modelManager)
            self.init(
                backends: [system, kokoro],
                systemBackend: system,
                kokoroModelManagerBox: modelManager
            )
        } else {

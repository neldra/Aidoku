    ...
    func speak(text: String, voiceID: String?, rate: Float, utteranceID: Int) {
        ...
        if let inflight = prefetchTasks.removeValue(forKey: key) {
            synthesisTask = Task { [weak self] in
                ...
                // The prefetch task also writes its result into
                // `prefetchedSamples` (single-task design). We consume it here
                // via the in-flight branch, so drop that cached copy or a full
                // PCM buffer leaks until the session ends.
                self.prefetchedSamples.removeValue(forKey: key)

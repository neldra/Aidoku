...
final class KokoroSpeechBackend: SpeechSynthesisBackend {
    ...
    /// Curated English voice roster. `af_heart` ships bundled with the model
    /// download; the rest are fetched lazily on first synthesis (~500 KB each)
    /// via `KokoroAneResourceDownloader.ensureVoicePack`. Quality tier follows
    /// Hexgrad's published A/B/C/D/F matrix — only A and B grades are exposed
    /// here. American voices (`af_`/`am_`) are tagged `en-US`, British
    /// (`bf_`/`bm_`) `en-GB`.
    static let voiceCatalog: [SpeechVoice] = [
        SpeechVoice(id: "af_heart",    displayName: "Heart",    language: "en-US", quality: .premium),
        SpeechVoice(id: "af_bella",    displayName: "Bella",    language: "en-US", quality: .premium),
        SpeechVoice(id: "af_nicole",   displayName: "Nicole",   language: "en-US", quality: .enhanced),
        SpeechVoice(id: "af_sky",      displayName: "Sky",      language: "en-US", quality: .enhanced),
        SpeechVoice(id: "af_sarah",    displayName: "Sarah",    language: "en-US", quality: .enhanced),
        SpeechVoice(id: "am_fenrir",   displayName: "Fenrir",   language: "en-US", quality: .enhanced),
        SpeechVoice(id: "am_michael",  displayName: "Michael",  language: "en-US", quality: .enhanced),
        SpeechVoice(id: "am_puck",     displayName: "Puck",     language: "en-US", quality: .enhanced),
        SpeechVoice(id: "bf_emma",     displayName: "Emma",     language: "en-GB", quality: .enhanced),
        SpeechVoice(id: "bf_isabella", displayName: "Isabella", language: "en-GB", quality: .enhanced),
        SpeechVoice(id: "bm_george",   displayName: "George",   language: "en-GB", quality: .enhanced),
        SpeechVoice(id: "bm_fable",    displayName: "Fable",    language: "en-GB", quality: .enhanced),
    ]

    func availableVoices() -> [SpeechVoice] { Self.voiceCatalog }

    func prepare() async {
        _ = try? await ensureEngine()
    }

    /// Speculatively download `voiceID`'s pack so the first `speak` after a
    /// voice change does not block. Engine init is required first — without
    /// it, FluidAudio's model store has no cache directory bound. If the
    /// engine isn't ready yet we initialize it here on the same task; once
    /// it's resolved we hand off to `KokoroAneManager.prefetchVoice` which
    /// calls `ensureVoicePack` via the model store.
    func prepareVoice(_ voiceID: String) {
        Task { [weak self] in
            guard let self else { return }
            do {
                let engine = try await self.ensureEngine()
                try await engine.prefetchVoice(voiceID)
            } catch {
                kokoroLog.debug(
                    "voice prefetch skipped: \(voiceID, privacy: .public) error=\(error.localizedDescription, privacy: .public)"
                )
            }
        }
    }
    ...
    private static func synthesize(
        ...
        if g2p.coverage >= misakiCoverageThreshold
            && !g2p.phonemes.isEmpty
            && !g2p.hasUnresolvedTokens {
            return try await engine.synthesizeFromPhonemesDetailed(
                g2p.phonemes, voice: voice, speed: 1.0
            )
        }
        // Low coverage, empty Misaki output, or a word neither the lexicon nor
        // per-word G2P resolved → FluidAudio's whole-text G2P (which attempts
        // the whole chunk, including the OOV word, rather than dropping it).
